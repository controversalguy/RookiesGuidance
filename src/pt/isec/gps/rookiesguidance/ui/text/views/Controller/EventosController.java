package pt.isec.gps.rookiesguidance.ui.text.views.Controller;
...
import static pt.isec.gps.rookiesguidance.ui.text.views.ViewSwitcher.getScene;
                    ...
                    if (dialogButton == ok) {
                        try {
                            ...
                            if (!connDB.editaEvento(Integer.parseInt(eventosText.get(j).getText()),tipo.getText(), local.getText(), data_hora, LoginController.getNumero())) {
                                ToastMessage.show(getScene().getWindow(), "Não foi possível editar o evento!");
                                return null;
                            }
                        } catch (SQLException e) {
                            throw new RuntimeException(e);
